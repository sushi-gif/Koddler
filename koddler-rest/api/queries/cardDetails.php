...
if (isset($_COOKIE['rest-auth-cookie'])) {
    ...
    try {
        ...
        $stmt = $db->prepare("SELECT issue_date, expiry_date, name, surname, tag, status from cards inner join users on cards.user = users.user_id where users.user_id = :userId");
